...
if [ ! -d /dashparty-lib/dashparty_lib.egg-info ]; then
    cd /dashparty-lib; python3 setup.py develop; cd /
fi
if [ ! -d /dashparty-cli/dashparty_cli.egg-info ]; then
    cd /dashparty-cli; python3 setup.py develop; cd /
fi
...
if [ ! -f /root/.local/share/dashparty/counterparty.db ]; then
    echo "Downloading mainnet bootstrap DB..."
    dashparty-server bootstrap --quiet
fi
if [ ! -f /root/.local/share/dashparty/dashparty.testnet.db ]; then
    echo "Downloading testnet bootstrap DB..."
    dashparty-server --testnet bootstrap --quiet
fi
...
/usr/local/bin/dashparty-server ${PARAMS} ${COMMAND} &
PID=$!
wait $PID
trap - TERM INT
wait $PID
